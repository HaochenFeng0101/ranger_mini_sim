...
class BedTrolleySE2Controller:
    def __init__(self):
        rospy.init_node('bed_trolley_se2_controller')
        ...
        # Subscribers
        self.odom_sub = rospy.Subscriber('/bed_trolley/four_wheel_steering_controller/odom', Odometry, self.odom_callback)
        self.target_sub = rospy.Subscriber('/bed_trolley/position_cmd', Pose2D, self.target_callback)
        
        rospy.loginfo("Bed Trolley SE2 Controller initialized - Simple closed-loop control")
...
def main():
    try:
        controller = BedTrolleySE2Controller()
        rospy.spin()
