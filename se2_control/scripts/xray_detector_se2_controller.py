...
class XrayDetectorSE2Controller:
    def __init__(self):
        rospy.init_node('xray_detector_se2_controller')
        ...
        # Publishers
        self.twist_pub = rospy.Publisher('/xray_detector/four_wheel_steering_controller/cmd_vel', Twist, queue_size=1)
        self.steering_pub = rospy.Publisher('/xray_detector/four_wheel_steering_command', FourWheelSteeringStamped, queue_size=1)
        
        # Subscribers
        self.odom_sub = rospy.Subscriber('/xray_detector/four_wheel_steering_controller/odom', Odometry, self.odom_callback)
        self.target_sub = rospy.Subscriber('/xray_detector/position_cmd', Pose2D, self.target_callback)
        
        rospy.loginfo("X-ray Detector SE2 Controller initialized - Simple closed-loop control")
...
def main():
    try:
        controller = XrayDetectorSE2Controller()
        rospy.spin()
